...
function getTradeConfig(vix) {
  ...
  // TARGET revised based on evidence from all 11 actual trades:
  //
  // VIX>16 → 25% target (UNCHANGED — all wins used this and hit it)
  //   May14 (VIX 18.6): ₹177→₹221 target → hit ✅
  //   Jun3a (VIX 16.3): ₹244→₹305 target → hit ✅
  //
  // VIX 13-16 → 25% target (REDUCED from 35%)
  //   Jun11b (VIX 15.1): 35% target was hit — but 25% would also hit earlier ✅
  //   Jun4/Jun5c/Jun11a (VIX 15-16): 35% never hit, 25% also never hit
  //   The losses were wrong signals, not wrong targets
  //   Keeping 25% ensures we exit faster on right signals
  //
  // VIX ≤ 13 → 20% target (REDUCED from 40%)
  //   Jun10 (VIX 13.1): 40% target — option peaked +7.4% — never close
  //   Jun18 (VIX 13.1): 40% target — option peaked +6.8% — never close
  //   VIX<13 = LOW volatility = market expects SMALL moves
  //   Setting 40% target when market expects 60pt range is backwards logic
  //   20% = realistic exit on a calm low-VIX day when signal IS right
  if (vix <= 0) {
    return { allow: true, targetMult: 1.25, slMult: 0.75, label: 'VIX unknown (safe defaults, SL 25%)' };
  }
  if (vix > 19) {
    return { allow: false, targetMult: 1.25, slMult: 0.75, label: 'VIX ' + vix + ' > 19 (very expensive — skip)' };
  }
  if (vix > 16) {
    return { allow: true, targetMult: 1.25, slMult: 0.75, label: 'VIX ' + vix + ' (expensive — target 25%, SL 25%)' };
  }
  if (vix > 13) {
    return { allow: true, targetMult: 1.25, slMult: 0.75, label: 'VIX ' + vix + ' (normal — target 25%, SL 25%)' };
  }
  return { allow: true, targetMult: 1.20, slMult: 0.75, label: 'VIX ' + vix + ' (low — target 20%, SL 25%)' };
}
...
async function fetchMarketData() {
  try {
    ...
    if (pc.length >= 1) {
      // Upstox daily API returns COMPLETED trading days only (no partial today)
      // pc[0] = yesterday (last completed trading day) ← correct for PDH/PDL
      // pc[1] = 2 trading days ago ← WRONG (was used before, caused 1-day lag)
      // Evidence: Jun15 showed PDH=Jun11 high, Jun16 showed PDH=Jun12 high
      // Fix: use pc[0] everywhere for prevClose, PDH, PDL
      state.prevClose = pc[0][4];
      // Store PDH and PDL from yesterday's candle
      // pc[0] = [timestamp, open, high, low, close, volume] = yesterday
      // These are the key price action levels for today's session
      const newPdh = pc[0][2];  // yesterday's high
      const newPdl = pc[0][3];  // yesterday's low
      const newPdDate = new Date(pc[0][0]).toISOString().slice(0,10);
