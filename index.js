...
async function enterTrade(signal) {
  ...
  try {
    ...
    const tradeTag  = isMorning ? 'MORNING' : isCrossover ? 'CROSSOVER' : 'MOMENTUM';
...
async function scan() {
  ...
  if (!state.position && !state.inTrade) {
    ...
    if (state.pdh && state.pdl && state.nifty > 0) {
  ...
  try {
    ...
    if (ltp > 0) {
      ...
      if (pos.paSlNifty !== null && state.nifty > 0) {
        ...
        // PA Target 1: NIFTY reached VWAP level (level-to-level exit)
        if (pos.paTarget1 !== null && !pos.paTarget1Hit) {
          const target1Hit = pos.direction === 'PE'
            ? nifty <= pos.paTarget1
            : nifty >= pos.paTarget1;
          if (target1Hit) {
            pos.paTarget1Hit = true;
            log('PA TARGET1 (VWAP): NIFTY ₹' + nifty + ' reached ₹' + pos.paTarget1 +
                ' — exiting at VWAP level ✅', 'trade');
            await exitPosition(pos, ltp, 'PA_TARGET1_VWAP');
            return;
          }
        }
      }
