...
http.createServer(async (req, res) => {
  ...
  // Browser calls POST /download with {type, from, to}
  // Server fetches from Upstox day by day, returns CSV
  if (req.method === 'POST' && req.url === '/download') {
    ...
    req.on('end', async () => {
      try {
        ...
        // Map type to Upstox path
        const paths = {
          nifty_daily:     (d) => `/v3/historical-candle/NSE_INDEX%7CNifty%2050/days/1/${d}/${d}`,
          nifty_5min:      (d) => `/v3/historical-candle/NSE_INDEX%7CNifty%2050/minutes/5/${d}/${d}`,
          nifty_15min:     (d) => `/v3/historical-candle/NSE_INDEX%7CNifty%2050/minutes/15/${d}/${d}`,
          nifty_30min:     (d) => `/v3/historical-candle/NSE_INDEX%7CNifty%2050/minutes/30/${d}/${d}`,
          banknifty_5min:  (d) => `/v3/historical-candle/NSE_INDEX%7CNifty%20Bank/minutes/5/${d}/${d}`,
          banknifty_15min: (d) => `/v3/historical-candle/NSE_INDEX%7CNifty%20Bank/minutes/15/${d}/${d}`,
        };

        const getPath = paths[type];
        if (!getPath) { send(400, { error: 'Unknown type: ' + type }); return; }

        // Fetch all days and build CSV
        const rows = ['date,time,open,high,low,close,volume'];
        let fetched = 0;
        ...
        res.end(csv);
        log('Download: ' + type + ' ' + from + ' to ' + to + ' — ' + fetched + ' candles', 'info');
