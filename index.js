...
function getTradeConfig(vix) {
  if (vix <= 0) {
    // VIX unknown — use safe defaults
    return { allow: true, targetMult: 1.35, slMult: 0.82, label: 'VIX unknown (safe defaults)' };
  }
  if (vix > 19) {
    // Top 10% most expensive options — skip trade
    return { allow: false, targetMult: 1.40, slMult: 0.82, label: 'VIX ' + vix + ' > 19 (very expensive — skip)' };
  }
  if (vix > 16) {
    // 75th-90th percentile — options expensive, exit faster
    return { allow: true, targetMult: 1.25, slMult: 0.83, label: 'VIX ' + vix + ' (expensive — target 25%)' };
  }
  if (vix > 13) {
    // 50th-75th percentile — normal pricing
    return { allow: true, targetMult: 1.35, slMult: 0.82, label: 'VIX ' + vix + ' (normal — target 35%)' };
  }
  // Below median — cheap options, hold for full target
  return { allow: true, targetMult: 1.40, slMult: 0.82, label: 'VIX ' + vix + ' (cheap — target 40%)' };
}
...
async function enterTrade(signal) {
  ...
  try {
    ...
    // ── STEP 0: NIFTY DISTANCE CHECK — block chasing exhausted moves ──
    // Check how far NIFTY has already moved in signal direction
    // over the last 6 candles (30 minutes).
    // If > 120pts already moved: the move is likely done. Don't enter.
    // Evidence: June 5 Trade1=224pts, Trade2=142pts, June 4=109pts — all lost
    //           June 3 wins: move was ongoing at entry, not completed
    if (state.candles5m && state.candles5m.length >= 6) {
      const recentCandles = state.candles5m.slice(-6);
      const oldest6Close  = recentCandles[0][4];  // close of oldest in window
      const current       = state.nifty;
      const niftyMove     = current - oldest6Close; // positive = rose, negative = fell
      // For CE: niftyMove should be positive (rose). Check if > 120pts.
      // For PE: niftyMove should be negative (fell). Check if < -120pts.
      const moveInSignalDir = signal.signal === 'CE' ? niftyMove : -niftyMove;
      if (moveInSignalDir > 120) {
        log('DISTANCE BLOCK: NIFTY already moved ' + Math.round(moveInSignalDir) + 'pts in ' +
            signal.signal + ' direction over last 30min — move likely exhausted', 'warn');
        state.inTrade = false; return;
      }
      log('Distance check: ' + Math.round(moveInSignalDir) + 'pts in ' + signal.signal + ' dir (max 120) ✅', 'info');
    }
    ...
    let fd = sr?.body?.data || {};
    // Retry once if average_price not yet populated (MARKET orders can take longer)
    ...
    log('FILL: ₹' + avgPrice + ' (est ₹' + chainPrice + ')' + (fd.average_price ? '' : ' ⚠️ used estimate'), 'trade');
