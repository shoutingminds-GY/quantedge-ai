...
http.createServer(async (req, res) => {
  ...
  // Browser calls POST /download with {type, from, to, token}
  // Server fetches from Upstox in MONTH chunks (v3 allows 1 month per request
  // for minute intervals), returns CSV. Avoids same-date edge case.
  if (req.method === 'POST' && req.url === '/download') {
    let body = '';
    req.on('data', c => body += c);
    req.on('end', async () => {
      try {
        const { type, from, to, token: dlToken } = JSON.parse(body);
        if (!dlToken) { send(400, { error: 'No token' }); return; }

        // Map type to instrument key + unit/interval
        const config = {
          nifty_daily:     { key: 'NSE_INDEX%7CNifty%2050',   unit: 'days',    interval: '1'  },
          nifty_5min:      { key: 'NSE_INDEX%7CNifty%2050',   unit: 'minutes', interval: '5'  },
          nifty_15min:     { key: 'NSE_INDEX%7CNifty%2050',   unit: 'minutes', interval: '15' },
          nifty_30min:     { key: 'NSE_INDEX%7CNifty%2050',   unit: 'minutes', interval: '30' },
          banknifty_5min:  { key: 'NSE_INDEX%7CNifty%20Bank', unit: 'minutes', interval: '5'  },
          banknifty_15min: { key: 'NSE_INDEX%7CNifty%20Bank', unit: 'minutes', interval: '15' },
        };

        const cfg = config[type];
        if (!cfg) { send(400, { error: 'Unknown type: ' + type }); return; }

        // Build month-sized chunks (v3 max retrieval = 1 month for minute data)
        // Each chunk: [chunkFrom, chunkTo] where chunkTo > chunkFrom
        const chunks = [];
        let chunkStart = new Date(from);
        const finalEnd = new Date(to);
        while (chunkStart <= finalEnd) {
          // chunk end = last day of this month, or finalEnd, whichever sooner
          const monthEnd = new Date(chunkStart.getFullYear(), chunkStart.getMonth() + 1, 0);
          const chunkEnd = monthEnd < finalEnd ? monthEnd : finalEnd;
          chunks.push([
            chunkStart.toISOString().slice(0, 10),
            chunkEnd.toISOString().slice(0, 10),
          ]);
          // next chunk starts first day of next month
          chunkStart = new Date(chunkStart.getFullYear(), chunkStart.getMonth() + 1, 1);
        }

        // Header depends on whether it's daily or intraday
        const header = cfg.unit === 'days'
        ...
        for (const [cFrom, cTo] of chunks) {
          // v3 path: /v3/historical-candle/{key}/{unit}/{interval}/{to}/{from}
          const apiPath = '/v3/historical-candle/' + cfg.key + '/' + cfg.unit + '/' + cfg.interval + '/' + cTo + '/' + cFrom;
          const result = await new Promise((resolve) => {
            ...
            const r = https.request(opts, (res) => {
              ...
              res.on('end', () => {
                ...
                catch(e) { resolve({ _parseError: data.slice(0, 200) }); }
              });
            });
            r.on('error', (e) => resolve({ _error: e.message }));
            r.setTimeout(15000, () => { r.destroy(); resolve({ _error: 'timeout' }); });
            r.end();
          });

          if (result._error || result._parseError) {
            log('Download chunk error ' + cFrom + ': ' + (result._error || result._parseError), 'error');
            continue;
          }

          const candles = result?.data?.candles || [];
          // v3 returns newest first — reverse to chronological
          for (const c of [...candles].reverse()) {
            if (cfg.unit === 'days') {
              rows.push(c[0].slice(0,10) + ',' + c[1] + ',' + c[2] + ',' + c[3] + ',' + c[4] + ',' + c[5]);
            } else {
              rows.push(c[0].slice(0,10) + ',' + c[0].slice(11,16) + ',' + c[1] + ',' + c[2] + ',' + c[3] + ',' + c[4] + ',' + c[5]);
            }
          }
        ...
        if (fetched === 0) {
          send(404, { error: 'No data returned from Upstox. Check token is valid and dates are trading days.' });
          return;
        }
        ...
        res.end(csv);
        log('Download: ' + type + ' ' + from + ' to ' + to + ' — ' + fetched + ' candles in ' + chunks.length + ' chunks', 'info');
