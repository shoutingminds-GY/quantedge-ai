...
async function enterTrade(signal) {
  ...
  try {
    // ── STEP 1: IV CHECK — is this a good time to buy? ─────────
    const tradeConfig = getTradeConfig(state.indiaVix);
    ...
    if (state.isExpiryDay) {
      const ist = istMins();
      // On Monday expiry: only trade before 12:00 PM (720 mins)
      // After noon theta decay is too severe
      if (ist > 720) {
        log('EXPIRY BLOCK: Past 12:00 PM on expiry day — theta too high', 'warn');
        state.inTrade = false; return;
      }
      // On expiry: require score 70+ (higher bar)
      if (signal.score < 70) {
        log('EXPIRY BLOCK: Score ' + signal.score + ' < 70 needed on expiry day', 'warn');
        state.inTrade = false; return;
      }
      log('EXPIRY DAY: Score ' + signal.score + ' qualifies (≥70). Proceeding.', 'info');
    }
    ...
    // On expiry day: cap max hold to 45 minutes
    const maxHoldMs = state.isExpiryDay ? 45 * 60 * 1000 : null;
    ...
    state.tradeCount++;
    log('OPEN: NIFTY ' + match.strike_price + ' ' + signal.signal +
        ' entry ₹' + avgPrice +
        ' SL ₹' + sl +
        ' target ₹' + tgt +
        ' (' + Math.round((tradeConfig.targetMult - 1) * 100) + '% target, VIX ' + state.indiaVix + ')' +
        (state.isExpiryDay ? ' [MAX 45min]' : ''), 'trade');
